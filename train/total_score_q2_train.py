...
df = df.drop(columns=['season', 'score_away', 'score_home', 'home_winner',"score_total",
                      "q1_away",	"q3_away",	"q4_away",	"ot_away",
                        	"q1_home",	"q3_home",	"q4_home",	"ot_home"
                      #, 'away_rat', 'home_rat'
...
df["q2_total"] = df["q2_home"] + df["q2_away"]
df = df.drop(columns=["q2_home", "q2_away"])
...
X_train = df.drop(columns=['q2_total'])
y_train = df['q2_total']
...
joblib.dump(best_model_obj, f"total_score_q2_pkls/total_score_q2_{best_model_name.replace(' ', '_').lower()}_model.pkl", compress=("lzma", 9))
joblib.dump(X_train.columns.tolist(), "total_score_q2_pkls/total_score_features.pkl")
joblib.dump(le_away, 'total_score_q2_pkls/total_score_label_encoder_away.pkl')
joblib.dump(le_home, 'total_score_q2_pkls/total_score_label_encoder_home.pkl')
print(f"\nSaved best model as: total_score_q2_{best_model_name.replace(' ', '_').lower()}_model.pkl")
